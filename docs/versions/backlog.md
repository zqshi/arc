...
> 最后更新: 2026-07-02 (v6.24 strict gate 死循环 P0 修复 done — P0+P1+P2 全清, 见 v6.24.0-snapshot.md「交付内容」; R5 pipeline + conversation BaaS 端到端已验证。strict gate 原标 v6.25 提前到 v6.24 完成)
...
| conversation 模式 BaaS provision 端到端未运行态实测 (send_message→execution_engine→artifact_extractor→try_provision_baas 仅代码路径确认, pipeline 路径已实测 provisioned=true, conversation 路径未跑真实 LLM 端到端) | P2 | v6.24 R5 验证 | ✅ 2026-07-02 已验证: free unified conversation 发消息→产出 tech_architecture→自动 extract 3 聚合→自动触发 provision→建表 orders/payments/inventories + RLS 4/表 + user_id auth.uid()。修复 2 阻断 bug: ① execution_engine sandbox_policy.target→build_target (AttributeError 阻断 conversation LLM 调用) ② convert_to_baas_schema 加 _slugify_identifier 容错中文 name (LLM 产 "Order（订单聚合根）" 致 _IDENT_RE 拒绝)。详见 v6.24.0-snapshot.md「交付内容」 |
| DomainModelExtractor._entities_to_aggregates 不清洗 entity name (原样保留 LLM 产出含中文/括号 "Order（订单聚合根）", convert 已边界容错 _slugify_identifier 但 domain_model 数据仍脏; 治本需 extractor 清洗为标识符+原描述移 description, 评估消费者后修) | P1 | v6.24 conversation 实测 | ⏳ 待治本: extractor 清洗 name, 评估 aggregate_relations 匹配/UI 展示影响 |
