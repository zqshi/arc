/**
 * 原型预览工具 — 单 artifact 的 Blob URL 多页面预览。
 * 从 ConversationModeView 提取。用于降级场景（无 prototype-bundle API 时）。
 */
...
function render(){document.getElementById('frame').src=urls[cur];document.getElementById('page-title').textContent=names[cur];document.getElementById('btn-back').disabled=hIdx<=0;renderPageList();}
function navigateTo(i){if(i<0||i>=urls.length||i===cur)return;cur=i;hist=hist.slice(0,hIdx+1);hist.push(i);hIdx=hist.length-1;render();}
function goBack(){if(hIdx>0){hIdx--;cur=hist[hIdx];render();}}
function findPage(text,href){var t=text.toLowerCase().replace(/\\s/g,'');var h=(href||'').toLowerCase();var best=-1,bestScore=0;for(var i=0;i<names.length;i++){var n=names[i].replace(/页$/,'').replace(/面$/,'').toLowerCase();if(!n)continue;var score=0;if(t===n||t===names[i].toLowerCase())score=100;else if(t.includes(n)||n.includes(t))score=80;else{for(var ci=0;ci<n.length;ci++){if(t.includes(n[ci]))score+=10;}}var hmap={'login':'登录','register':'注册','signup':'注册','home':'首页','dashboard':'工作台','list':'列表','detail':'详情','chat':'对话','setting':'设置','profile':'个人'};for(var k in hmap){if(h.includes(k)&&names[i].includes(hmap[k]))score+=60;}if(score>bestScore){bestScore=score;best=i;}}if(best>=0&&bestScore>=20)return best;return(cur+1)%urls.length;}
function togglePages(){document.getElementById('page-list').classList.toggle('open');}
function renderPageList(){var el=document.getElementById('page-list');el.innerHTML=names.map(function(n,i){return '<button class="'+(i===cur?'active':'')+'" onclick="navigateTo('+i+');togglePages()">'+n+'</button>';}).join('');}
function showToast(msg){var el=document.getElementById('toast');el.textContent=msg;el.classList.add('show');setTimeout(function(){el.classList.remove('show');},1500);}
window.addEventListener('message',function(e){if(e.data&&e.data.type==='nav'){var idx=findPage(e.data.text,e.data.href);if(idx===cur){showToast('当前已在: '+names[cur]);}else{navigateTo(idx);showToast('跳转到: '+names[idx]);}}});
document.addEventListener('click',function(e){if(!e.target.closest('.page-list,.pages-btn'))document.getElementById('page-list').classList.remove('open');});
render();
</script></body></html>`;
