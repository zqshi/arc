...
# JDK 17 + Android SDK (platform/build-tools/NDK) + Node 20 + Gradle + capacitor-cli。
#
...
#
# 基础镜像选 eclipse-temurin:17-jdk (glibc, 官方 JDK17):
# Android SDK 预编译二进制 (aapt2/d8/NDK toolchain) 依赖 glibc, alpine/musl 兼容差。
# 强制 --platform=linux/amd64: Android build-tools 的 aapt2 等是 x86_64 ELF, 无 arm64 linux 版;
# x86_64 CI/服务器原生跑; Apple Silicon 经 Rosetta 翻译整个 amd64 镜像 (含 x86 ld-linux + 原生 x86 aapt2)。
#
# 构建: 见同目录 Makefile (make android-builder, ~10-20min, NDK 下载占大头)
